...
def index():
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            generator_animation_filename = f'generator_{filename}'
            optimizer_animation_filename = f'optimizer_{filename}'
            ...
            return redirect(url_for('show_animations', filename=filename, demo='upload'))
    return render_template('index.html')


@app.route('/animations/<filename>/<demo>', methods=['GET', 'POST'])
def show_animations(filename, demo):
    generator_animation_filename = f'generator_{filename}'
    optimizer_animation_filename = f'optimizer_{filename}'
    if request.method == 'POST':
...
def animated_svg_genetic(animation_filename_genetic, demo):
    print(f'################{demo}')
    if demo:
        return send_from_directory(app.config['UPLOAD_FOLDER'],
                                   'generator_example.svg')
    else:
        return send_from_directory(app.config['UPLOAD_FOLDER'],
...
def animated_svg_entmoot(animation_filename_entmoot, demo):
    print(f'################{demo}')
    if demo:
        return send_from_directory(app.config['UPLOAD_FOLDER'],
                                   'optimizer_example.svg')
    else:
        return send_from_directory(app.config['UPLOAD_FOLDER'],
...
@app.route('/redirect_button_demo', methods=['GET'])
def redirect_button_demo():
    return redirect(url_for('show_animations', filename='example.svg', demo='demo'))
